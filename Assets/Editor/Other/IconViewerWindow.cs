    ...
    public class IconViewerWindow : EditorWindow
    {
        ...
        private void OnEnable()
        {
            ...
            icons = Resources.FindObjectsOfTypeAll(typeof(Texture2D))
                .Where(x => AssetDatabase.GetAssetPath(x) ==
                            "Library/unity editor resources") //アイコンのAssetPathを取得すると全てこれ
        ...
        private void OnGUI()
        {
            ...
            if (evt.type == EventType.Repaint)
            {
                if (blackLabel == null)
                {
                    blackLabel = new GUIStyle(GUI.skin.label)
                        { richText = true, alignment = TextAnchor.MiddleLeft, padding = new RectOffset(5, 0, 0, 0) };
                }
            ...
            var colorPickerRect = new Rect(iconHeaderRect.xMax + iconRectWidth - 100,
                headerRect.y + (headerHeight - EditorGUIUtility.singleLineHeight) / 2, 100,
            ...
            foreach (var icon in icons)
            {
                ...
                var elementRect = EditorGUILayout.GetControlRect(GUILayout.Width(labelWidth + iconRectWidth * 2),
                    GUILayout.Height(iconRectHeight + (foldouts[icon.name] ? copyRectHeight + copyRectPadding : 0)));
                ...
                var iconRect = new Rect(labelRect.xMax + iconPadding.left, elementRect.y + iconPadding.top, icon.width,
                    icon.height);
                var darkRect = new Rect(labelRect.xMax + iconRectWidth, elementRect.y, iconRectWidth,
                    elementRect.height);
                var darkIconRect = new Rect(darkRect.x + iconPadding.left, elementRect.y + iconPadding.top, icon.width,
                    icon.height);

                if (foldouts[icon.name])
                {
                    EditorGUI.DrawRect(new Rect(elementRect.position - Vector2.one, elementRect.size + Vector2.one * 2),
                ...
                if (foldouts[icon.name])
                {
                    var foldedRect = new Rect(elementRect.x + copyRectPadding, elementRect.y + iconRectHeight,
                        elementRect.width - copyRectPadding * 2, copyRectHeight);
                    var copyButton1Rect = new Rect(foldedRect.x + 2, foldedRect.y + foldedRectLinePaddingHeight, 60,
                        foldedRectLineHeight);
                    var copyContent1Rect = new Rect(foldedRect.x + 70, foldedRect.y + foldedRectLinePaddingHeight,
                        foldedRect.width - 80, foldedRectLineHeight);
                    var copyButton2Rect = new Rect(foldedRect.x + 2,
                        foldedRect.y + foldedRectLineHeight + foldedRectLinePaddingHeight * 3, 60,
                        foldedRectLineHeight);
                    var copyContent2Rect = new Rect(foldedRect.x + 70,
                        foldedRect.y + foldedRectLineHeight + foldedRectLinePaddingHeight * 3, foldedRect.width - 80,
                        foldedRectLineHeight);
                    var widthRect = new Rect(foldedRect.xMax - 190, foldedRect.y + foldedRectLinePaddingHeight, 90,
                        foldedRectLineHeight);
                    var heightRect = new Rect(foldedRect.xMax - 190,
                        foldedRect.y + foldedRectLineHeight + foldedRectLinePaddingHeight * 3, 90,
                        foldedRectLineHeight);
                    var exportRect = new Rect(foldedRect.xMax - 80,
                        foldedRect.y + foldedRectLineHeight / 2 + foldedRectLinePaddingHeight * 2, 70,
                        foldedRectLineHeight);
                    EditorGUI.DrawRect(new Rect(foldedRect.position - Vector2.one, foldedRect.size + Vector2.one * 2),
                    ...
                    GUI.Label(copyContent2Rect,
                        $"(<color=#4ec9b0>Texture2D</color>)<color=#4ec9b0>EditorGUIUtility</color>.<color=#dcdcaa>Load</color>(<color=#d69d85>\"{icon.name}\"</color>)",
                        whiteLabel);
                    ...
                    {
                        var path = EditorUtility.SaveFilePanel($"Export the icon [{icon.name}]", Application.dataPath,
                            icon.name, "png");
        ...
        /// </summary>
        public static string FilterField(string filter, System.Action repaintCallback,
            string controlName = "__FilterField__")
        {
            var evt = Event.current;
            using (new EditorGUILayout.HorizontalScope())
            {
                //入力中にEnterキーでフォーカスを外す
                if (GUI.GetNameOfFocusedControl() == controlName && evt.type == EventType.KeyDown &&
